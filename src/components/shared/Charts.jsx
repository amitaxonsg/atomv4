...
export function RadarChart({ values, labels }) {
  const markers = labels.map((_, index) => String.fromCharCode(65 + index));
  ...
  return <svg className="radar" viewBox="0 0 240 240" role="img" aria-label="Subscale radar chart A through J, scores range from 5 to 25">
    ...
    {markers.map((label, index) => { const angle = Math.PI * 2 * index / markers.length - Math.PI / 2; return <text key={label} x={120 + Math.cos(angle) * 108} y={123 + Math.sin(angle) * 108}>{label}</text>; })}
  </svg>;
}
